        ...
        <header className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-mono uppercase tracking-widest leading-none">Security Protocol · {dashboardData.last_updated}</p>
            </div>
          </div>
          ...
          {activeTab === 'overview' ? (
                ...
                <div className="flex justify-between items-end pt-4">
                  <div className="flex items-center space-x-3">
                    <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-xs font-bold border border-emerald-100 flex items-center">
                      {p(dashboardData.performance["1d"])} <span className="ml-1 font-mono opacity-50 uppercase">Snapshot</span>
                    </div>
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-slate-800 text-lg font-bold">Recorded Holdings</h2>
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-50/50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">Price</th>
                  ...
                  <tbody>
                    {dashboardData.holdings.map((h: any, idx: number) => (
                      <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-5 border-b border-slate-50 text-right text-xs font-mono font-medium text-slate-400">${p(h.price)}</td>
